...
def load_args():
    ap = argparse.ArgumentParser(description='Create tfrecord to be used in 2D CNN models.')
    ...
    ap.add_argument('-o', '--output-path',
                                    dest='output_path',
                                    help='path to output the extracted frames.',
                                    type=str, required=False, default='/DL/UCF-101/tfrecords/')
...
def _get_dataset_filename(output_path, split_name, shard_id, num_shards):
  output_filename = 'ucf101_%s_%05d-of-%05d.tfrecord' % (
...
# Get blacklist
  ...
  if mode == 'TRAIN' :
    ...
    for m  in train_metadata:
        train_class[m[1]] += 1
    for m  in validation_metadata:
        validation_class[m[1]] += 1
    ...
    random.shuffle(validation_metadata)
    random.shuffle(test_metadata)

    print('INFO: train_len 2', len(train_metadata), file=sys.stderr)
    print('INFO: validation_len 2', len(validation_metadata), file=sys.stderr)
    print('INFO: test_len 2', len(test_metadata), file=sys.stderr)

    # Convert the train_metadata and validation_metadata sets.
    if 'train' not in excluded_scope:
        _convert_dataset('train', train_metadata, dataset_dir, output_dir)
    if 'validation' not in excluded_scope:
        _convert_dataset('validation', validation_metadata, dataset_dir, output_dir)
    if 'test' not in excluded_scope:
        _convert_dataset('test', test_metadata, dataset_dir, output_dir)

    # Saves classes and split sizes
    print('INFO: ', CLASSES_TO_SIZES, file=sys.stderr)
    pickle.dump(CLASSES_TO_SIZES, open(os.path.join(output_dir, 'classes_to_sizes.pkl'), 'wb'))

    SPLITS_TO_SIZES = { 'train'      : len(train_metadata),
