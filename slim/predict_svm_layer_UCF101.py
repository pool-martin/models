...
print('{} {} {}'.format(image_ids[0], labels[0], features[0]))
...
#    labels_j = (labels == j).astype(np.int)
    acc = sk.metrics.accuracy_score(labels, predictions_j.np.where(v == maximum)(np.max(predictions_j)))
    print('Binary Acc: ', acc, file=metfile)
    accs.append(acc)
#    auc = sk.metrics.roc_auc_score(labels, scores_j.index(max(predictions_j)))
#    aucs_s.append(auc)
#    print('AUC scores [%d]: ' % j, auc, file=metfile)
#    auc = sk.metrics.roc_auc_score(labels, predictions_j.index(max(predictions_j)))
#    aucs_p.append(auc)
#    print('AUC predictions [%d]: ' % j, auc, file=metfile)
#    mAP = sk.metrics.average_precision_score(labels, scores_j.index(max(predictions_j)))
#    mAPs_s.append(mAP)
#    print('mAP scores [%d]: ' % j, mAP, file=metfile)
#    mAP = sk.metrics.average_precision_score(labels, predictions_j.index(max(predictions_j)))
#    mAPs_p.append(mAP)
...
#  tn, fp, fn, tp = sk.metrics.confusion_matrix(labels, predictions_m.index(max(predictions_m))).ravel()
