...
if [ "$5" == "" ]; then
    echo usage: predict_component_model_isbi.sh [ INCEPTION | RESNET ] checkpoint_path test_dataset split_name prediction_file
    exit 1
fi
...
if [ "$MODEL" == "INCEPTION" ]; then
    python "$ISBI_MODELS"/predict_image_classifier.py \
        ...
        --id_field_name=id \
        --batch_size="$BATCH" \
        --output_file="$TEMP_DIR"/allpredict.tmp \
        "$@"
else
    python "$ISBI_MODELS"/predict_image_classifier.py \
        --alsologtostderr \
...
echo Assembling "$DATASET_NAME" into "$PREDICTION_FILE"

cut -d, -f1,4,5 < "$TEMP_DIR"/allpredict.tmp | tail -n +2 | tr -d ' ' | sort > "$PREDICTION_FILE"