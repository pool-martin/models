...
def read_pickled_data(input_training):
    ids =[]
    num_samples = 0
...
#                print(line, file=sys.stderr)
            ...
            for j in xrange(1, 1025):
                ...
                if np.isnan(value):
                    ...
                    row[j-1] = value
            features[i] = row
            if np.isnan(np.sum(features[i])):
                print('nan at ', i, 'feat :', features[i], file=sys.stderr)
            
            if(i % 1000 == 0):
                print('reading i=', i, file=sys.stderr)
            i += 1
    k = 0
    print('CHecking for NaN', file=sys.stderr)
    for feat in features:
        if np.isnan(np.sum(feat)):
            print('nan at ', k, file=sys.stderr)
        k += 1
...
ids, labels, features = read_pickled_data(FLAGS.input_training)
